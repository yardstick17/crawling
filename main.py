	...
	for link in restaurant_section:
		for sub_link in link.find_all('h2' , {"class":"restro_title"}):
			for l in sub_link.find_all('a'):
				x = l.get('href')
				sub_page_link.append('https://www.eazydiner.com' + str(x.encode('utf-8')) )
...
def get_deals_detail(current_page , sub_page_link):
	...
	for link in sub_page_link:
		if not os.path.isfile('bengaluru/' + str(current_page) + '_'+  str(total_scanned) + '_Delhi_NCR.json'):
			try:
				...
				price_range = soup.find_all('div' , class_ = 'details_pline area price')
				for sec in price_range:
					for price in sec.find_all('div' , {"itemprop":"priceRange"}):
						D['price_range'] = price.text.encode('utf-8').strip()


				restaurant_deatils = soup.find_all('div' , class_ = 'details_line')
				for detail in restaurant_deatils:
					for det in detail.find_all('span' ,{"itemprop":"addressRegion"}):
						D['city'] = det.text.encode('utf-8').strip()

					for det in detail.find_all('div' , {"itemprop":"streetAddress"}):
						D['address'] = det.text.encode('utf-8').strip()

					for det in detail.find_all('span' , {"itemprop":"telephone"}):
						Tele.append(det.text.encode('utf-8').strip())

					D['telephone_number'] = Tele

				for detail in restaurant_deatils:
					for det in detail.find_all('div' , {"class":"details_pline area"}):
				...
				deal_deatils = soup.find('div' , class_ = 'navs resturant_details')
				D['deal_deatils'] = deal_deatils.text.encode('utf-8').strip().split('\n')
				final_restaurant.append(D)

				print 'Res Details : ' , D
				with open('bengaluru/' + str(current_page) + '_'+  str(total_scanned) + '_bengaluru.json', 'w') as outfile:
			   		json.dump(D, outfile)
			except:
				sleep(10)
				print 'Err...'
